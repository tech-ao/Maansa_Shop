(function ($) {
    ...
    $(document).ready(function () {
        ...
        function scrollToActiveSidebar() {
            ...
            if ($activeEl.length) {
                var $container = $('.sidebar .sidebar-wrapper, .sidebar .scroll-content').first();
                if ($container.length) {
                    var containerTop = $container.offset().top;
                    var containerHeight = $container.height();
                    var elTop = $activeEl.offset().top;
                    var currentScroll = $container.scrollTop();

                    // Center the active item in the sidebar viewport
                    var targetScroll = currentScroll + (elTop - containerTop) - (containerHeight * 0.3);
        ...
        // 3. Save scroll position on sidebar link clicks
    ...
    // Universal Tab Persistence across Page Reloads & Form Submissions
    $(document).on('shown.bs.tab', 'a[data-toggle="tab"], a[data-toggle="pill"]', function (e) {
        var hash = $(e.target).attr('href');
        if (hash && hash.startsWith('#')) {
            var storageKey = 'active_tab_' + window.location.pathname;
    ...
    $(function () {
        var storageKey = 'active_tab_' + window.location.pathname;
        var activeTab = window.location.hash || sessionStorage.getItem(storageKey);
        if (activeTab && activeTab.startsWith('#')) {
            var targetTab = $('a[data-toggle="tab"][href="' + activeTab + '"], a[data-toggle="pill"][href="' + activeTab + '"]');
            if (targetTab.length) {
                targetTab.tab('show');
            }
        }
