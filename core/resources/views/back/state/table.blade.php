    ...
    <tr>
        ...
        <td class="text-center" style="min-width: 140px;">
            @if ($data->type == 'fixed')
                <span class="badge px-3 py-1 font-weight-bold" style="background: #dcfce7; color: #15803d; border: 1px solid #bbf7d0; font-size: 12.5px; border-radius: 8px;">
                    <i class="fa-solid fa-tag mr-1"></i> {{ PriceHelper::adminCurrencyPrice($data->price) }}
                </span>
            @else
                <span class="badge px-3 py-1 font-weight-bold" style="background: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd; font-size: 12.5px; border-radius: 8px;">
                    <i class="fa-solid fa-percent mr-1"></i> {{ $data->price }}%
                </span>
            @endif
        </td>
        ...
        <td class="text-center" style="min-width: 115px;">
            <div class="dropdown d-inline-block">
                ...
                <div class="dropdown-menu animated--fade-in shadow border-0" aria-labelledby="dropdownStatus-{{ $data->id }}" style="border-radius: 12px; min-width: 140px;">
                    <h6 class="dropdown-header text-muted font-weight-bold" style="font-size: 11px; text-transform: uppercase;">{{ __('State Rule Status') }}</h6>
                    <a class="dropdown-item {{ $data->status == 1 ? 'active font-weight-bold' : '' }}" href="{{ route('back.state.status', [$data->id, 1]) }}">
                        <i class="fa-solid fa-check text-success mr-2"></i> {{ __('Enable') }}
                    </a>
                    <a class="dropdown-item {{ $data->status == 0 ? 'active font-weight-bold' : '' }}" href="{{ route('back.state.status', [$data->id, 0]) }}">
        ...
        <td class="text-center" style="min-width: 100px;">
            <div class="action-btn-group justify-content-center">
                <a class="btn-action-icon btn-action-edit" href="{{ route('back.state.edit', [$data->id]) }}" title="{{ __('Edit State Rule') }}">
                    <i class="fa-solid fa-pen"></i>
                </a>
                <a class="btn-action-icon btn-action-delete" data-toggle="modal" data-target="#confirm-delete" href="javascript:;"
                    data-href="{{ route('back.state.destroy', [$data->id]) }}" title="{{ __('Delete State Rule') }}">
