    ...
    <div class="dash-hero-banner mb-4">
        <div class="dash-hero-content">
            <div class="dash-hero-text">
                <h2><i class="fa-solid fa-star-half-stroke mr-2" style="font-size: 22px;"></i> {{ __('Product Reviews & Ratings') }}</h2>
                <p>{{ __('Moderate, inspect, and approve customer ratings and product testimonials.') }}</p>
            </div>
				...
				<table class="table-modern" id="admin-table" width="100%" cellspacing="0">
					<thead>
						<tr>
							...
							<th width="13%" class="text-center no-sort" data-orderable="false">{{ __('Actions') }}</th>
						</tr>
					</thead>
					<tbody>
                        @include('back.review.table', compact('datas'))
					</tbody>
				</table>
			</div>
		</div>
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 16px; overflow: hidden;">
            <div class="modal-header bg-danger text-white border-0 py-3">
                <h5 class="modal-title d-flex align-items-center font-weight-bold" id="exampleModalLabel">
                    <i class="fas fa-exclamation-triangle mr-2"></i> {{ __('Confirm Review Deletion') }}
                </h5>
            ...
            <div class="modal-body p-4 text-center">
                ...
                <h5 class="font-weight-bold text-dark mb-2">{{ __('Delete This Review?') }}</h5>
                <p class="text-muted mb-0">
                    {{ __('You are going to delete this review. All ratings and customer feedback associated with this review will be removed.') }}
                </p>
            </div>
            <div class="modal-footer bg-light border-0 py-3 d-flex justify-content-center">
                ...
                <form action="" class="d-inline btn-ok" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger px-4" style="border-radius: 10px; font-weight: 700;">{{ __('Delete Review') }}</button>
                </form>
            </div>
        </div>
    </div>
