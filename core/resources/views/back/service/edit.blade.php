    ...
    <div class="dash-hero-banner mb-4">
        <div class="dash-hero-content">
            <div class="dash-hero-text">
                <h2><i class="fa-solid fa-pen-to-square mr-2" style="font-size: 22px;"></i> {{ __('Edit Service') }}</h2>
                <p>{{ __('Update service highlight details, icon graphic, and promotional description.') }}</p>
        ...
        <div class="col-xl-8 col-lg-9 col-12">
            <div class="card-modern">
                <div class="card-modern-body p-4">
                    ...
                    <form class="admin-form" action="{{ route('back.service.update', $service->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="row align-items-center mb-2">
                                ...
                                <div class="col-12 col-sm">
                                    <div class="position-relative mb-2">
                                        <label class="file">
                                            <input type="file" accept="image/*" class="upload-photo" name="photo" id="file_service" aria-label="Upload Service Image">
                                            <span class="file-custom text-left">{{ __('Change Service Icon...') }}</span>
                                        </label>
                                    </div>
                        ...
                        <div class="settings-section-card mb-4">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" name="title" class="form-control" id="title" placeholder="{{ __('e.g. Free Worldwide Shipping') }}" value="{{ $service->title }}" required>
                            </div>

                            <div class="form-group mb-0">
                                <label for="details" class="form-label font-weight-bold">{{ __('Details / Subtitle') }} *</label>
                                <textarea name="details" id="details" class="form-control" rows="4" placeholder="{{ __('e.g. On all orders over $100.00') }}" required>{{ $service->details }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end mt-4">
                            <button type="submit" class="btn btn-primary px-4 py-2" style="border-radius: 10px; font-weight: 700; background: linear-gradient(135deg, #10b981, #059669); border: none; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);">
                                <i class="fa-solid fa-floppy-disk mr-1"></i> {{ __('Update Service') }}
                            </button>
                        </div>
                    </form>
                </div>
