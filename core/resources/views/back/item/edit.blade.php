...
<div class="container-fluid">
    ...
    <div class="dash-hero-banner mb-4">
        <div class="dash-hero-content">
            <div class="dash-hero-text">
                <h2><i class="fa-solid fa-box-open mr-2" style="font-size: 22px;"></i> {{ __('Update Physical Product') }}</h2>
                <p>{{ __('Edit details, update product images, adjust pricing, stock quantities, and SEO metadata.') }}</p>
            </div>
    ...
    <!-- Main Form -->
    <form class="admin-form" action="{{ route('back.item.update',['item' => $item->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                ...
                <div class="card-modern mb-4">
                    ...
                    <div class="card-modern-body p-4">
                        <div class="form-group mb-4">
                            ...
                            <textarea name="sort_details" id="sort_details" class="form-control" rows="3" placeholder="{{ __('Brief highlight summary of this product...') }}">{{ $item->sort_details }}</textarea>
                        </div>

                        <div class="form-group mb-0">
                            <label for="details" class="form-label font-weight-bold text-dark">{{ __('Full Detailed Description') }} *</label>
                            <textarea name="details" id="details" class="form-control text-editor" rows="6" placeholder="{{ __('Enter full product description, specifications, and materials...') }}">{{ $item->details }}</textarea>
                        </div>
                    </div>
                </div>
