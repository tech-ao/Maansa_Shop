    ...
    <div class="dash-hero-banner mb-4">
        <div class="dash-hero-content">
            <div class="dash-hero-text">
                <h2><i class="fa-solid fa-pen-to-square mr-2" style="font-size: 22px;"></i> {{ __('Update Blog Category') }}</h2>
                <p>{{ __('Update blog category title, permalink slug, and publishing settings.') }}</p>
            </div>
		...
		<div class="col-lg-12">
			<div class="card-modern">
				<div class="card-modern-body">
					<form class="admin-form" action="{{ route('back.bcategory.update', $bcategory->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
							...
							<div class="col-md-6 mb-3">
								...
								<div class="input-group">
									...
									<input type="text" name="name" class="form-control item-name" id="name"
										placeholder="{{ __('Enter Name') }}" value="{{ $bcategory->name }}" required>
								</div>
							</div>
							...
							<div class="col-md-6 mb-3">
								...
								<div class="input-group">
									...
									<input type="text" name="slug" class="form-control" id="slug"
										placeholder="{{ __('Enter Slug') }}" value="{{ $bcategory->slug }}" required>
								</div>
							</div>
						</div>
						...
						<div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
							...
							<button type="submit" class="btn btn-primary px-4" style="border-radius: 10px; font-weight: 700; background: linear-gradient(135deg, #4f46e5, #3b82f6); border: none; box-shadow: 0 4px 12px rgba(79, 70, 229, 0.3);">
                                <i class="fa-solid fa-floppy-disk mr-1"></i> {{ __('Update Category') }}
                            </button>
						</div>
