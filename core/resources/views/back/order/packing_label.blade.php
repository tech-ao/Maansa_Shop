...
<head>
    ...
    <title>{{ __('Packing Label') }} #{{ $order->transaction_number }} - {{ $setting->title }}</title>
    <style>
        * {
...
<body onload="window.print()">
    @php
        ...
        // Determine primary recipient address (Shipping info if present, else Billing info)
    ...
    <div class="label-outer-container">
        <!-- Interactive Non-Print Navigation Bar -->
        <div class="no-print-bar">
            <div style="display: flex; gap: 8px; align-items: center;">
                <a href="{{ route('back.order.index') }}" class="btn-action btn-back">
                    &larr; {{ __('Orders') }}
                </a>
                <a href="{{ route('back.order.invoice', $order->id) }}" class="btn-action btn-invoice">
                    <svg style="width: 14px; height: 14px; margin-right: 5px; fill: currentColor;" viewBox="0 0 24 24"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg> {{ __('View Invoice') }}
                </a>
            </div>
            <div style="display: flex; gap: 8px; align-items: center;">
                <button onclick="window.print()" class="btn-action btn-print">
                    <svg style="width: 14px; height: 14px; margin-right: 5px; fill: currentColor;" viewBox="0 0 24 24"><path d="M19 8H5c-1.66 0-3 1.34-3 3v6h4v4h12v-4h4v-6c0-1.66-1.34-3-3-3zm-3 11H8v-5h8v5zm3-7c-.55 0-1-.45-1-1s.45-1 1-1 1 .45 1 1-.45 1-1 1zm-1-9H6v4h12V3z"/></svg> {{ __('Print Label') }}
                </button>
            </div>
        </div>
        ...
        <div class="packing-label-box">
            ...
            <div class="routing-strip">
                ...
                <div class="routing-cell payment-cell">
                    <div class="routing-label">{{ __('Payment Mode') }}</div>
                    @if($isCod)
                        <span class="badge-payment payment-cod">{{ __('COD') }} ({{ $order->currency_sign }}{{ PriceHelper::OrderTotal($order) }})</span>
                    @else
                        <span class="badge-payment payment-prepaid">{{ __('PREPAID') }}</span>
                    @endif
                </div>
            </div>
