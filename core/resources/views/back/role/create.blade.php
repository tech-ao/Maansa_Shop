    ...
    <div class="dash-hero-banner mb-4">
        <div class="dash-hero-content">
            <div class="dash-hero-text">
                <h2><i class="fa-solid fa-plus-circle mr-2" style="font-size: 22px;"></i> {{ __('Create New Role') }}</h2>
                <p>{{ __('Define a custom administrative role and assign module-level access permissions.') }}</p>
            </div>
		...
		<div class="col-lg-12">
			<div class="card-modern">
				<div class="card-modern-body">
					<form class="admin-form" action="{{ route('back.role.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                            ...
                            @foreach($groupPerms as $perm)
                                ...
                                <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 mb-3">
                                    <label class="perm-checkbox-card" for="perm_{{ $cardIndex }}">
                                        ...
                                        <input type="checkbox" name="section[]" value="{{ $perm['key'] }}" id="perm_{{ $cardIndex }}" class="perm-checkbox d-none" {{ (is_array(old('section')) && in_array($perm['key'], old('section'))) ? 'checked' : '' }}>
					...
					<div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top">
						...
						<button type="submit" class="btn btn-primary px-4" style="border-radius: 10px; font-weight: 700; background: linear-gradient(135deg, #10b981, #059669); border: none; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3);">
                            <i class="fa-solid fa-floppy-disk mr-1"></i> {{ __('Save Role') }}
                        </button>
					</div>
